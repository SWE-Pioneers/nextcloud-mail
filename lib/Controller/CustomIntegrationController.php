...
class CustomIntegrationController extends Controller {
	...
	public function configure(
		...
		if (empty($clientId) || empty($clientSecret) || empty($authorizationEndpoint)
			|| empty($tokenEndpoint) || empty($imapHost)) {
			return JsonResponse::fail(null, Http::STATUS_UNPROCESSABLE_ENTITY);
		}

		$this->customIntegration->configure(
			$clientId,
			$clientSecret,
